<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="题目类型" prop="timuleixing">
          <el-select v-model="ruleForm.timuleixing" placeholder="请选择题目类型">
            <el-option
                v-for="(item,index) in timuleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="题目类型" prop="timuleixing">
	      <el-input v-model="ruleForm.timuleixing"
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="答案" prop="daan">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.daan" label="答案" prop="daan">
                    <span>{{ruleForm.daan}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="题目" prop="timu">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="题目"
                  v-model="ruleForm.timu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.timu" label="题目" prop="timu">
                    <span>{{ruleForm.timu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.timu = true;
          ...
          if(o=='timuleixing'){
            this.ruleForm.timuleixing = obj[o];
	    this.ro.timuleixing = true;
            continue;
          }
          if(o=='wenjian'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jingsaitiku/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jingsaitikuCrossAddOrUpdateFlag = false;
                  this.parent.search();
